...
Reverted the heavier 32px header from 1.4.3 back to the slim CD-Tracker chrome that the suite shipped pre-harmonization — 22px tall, FRIZQT 12 (no outline), plain `×` close button, drop the fel-green underline. Kept the two-tone title color (`Wick's` in fel-green, `TBC BIS Tracker` in cream) since that's the part of the 1.4.3 work that worked.
